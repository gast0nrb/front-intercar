...
const Productos = ({
  ...
  useEffect(() => {
    const fetchProds = async () => {
      ...
      const productosFilter = productosFetch.data.Productos.filter(
        (p) => p.codigo != codigo
      );
        ...
        productos.data.Productos.length == 0 ? (
          <div className="text-center">
            ...
            <p className="text-gray-700 opacity-50">
              Aún no tenemos tantos productos en esta categoria para
              recomendarte...
            </p>
          </div>
        ) : (
          <>
            ...
            <WrapSimilares Productos={productos} base={base} />
            <ProductosReload
              title={reloadName}
              base={base}
              setBase={setBase}
              long={productos.data.Productos.length}
            />
          </>
        )
      ) : (
